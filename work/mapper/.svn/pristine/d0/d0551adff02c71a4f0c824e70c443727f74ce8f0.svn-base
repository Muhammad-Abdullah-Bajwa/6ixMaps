...
#include "graphics_types.h" //Needed by draw_surface to access the underlying surface type
...
#define T_AREA_HEIGHT 45  /* Height of text window */

#define BUTTON_TEXT_LEN 100
#define T_BOX2_WIDTH 400
...
static void force_settextattrs(int pointsize, double degrees);
...
void x11_build_textarea2(void);
//void x11_build_textarea3(bool draw);
...
void menu2text(XftDraw* draw, int xc, int yc, const char *text);
...
void open_tab() {
    init_graphics("Map", t_color(235, 235, 235, 255));
}
...
t_point scrn_to_world(const t_point& point1) {
    return t_point(
            xscrn_to_world(point1.x),
            yscrn_to_world(point1.y)
            );
}

t_bound_box scrn_to_world(const t_bound_box& box1) {
    return t_bound_box(
            scrn_to_world(box1.bottom_left()),
            scrn_to_world(box1.top_right())
            );
}
...
t_point world_to_scrn(const t_point& point1) {
    return t_point(
            xworld_to_scrn_fl(point1.x),
            yworld_to_scrn_fl(point1.y)
            );
}

t_bound_box world_to_scrn(const t_bound_box& box1) {
    return t_bound_box(
            world_to_scrn(box1.bottom_left()),
            world_to_scrn(box1.top_right())
            );
}
...
void force_setcolor(const t_color& new_color) {
...
void settextrotation(double degrees) {
...
void settextattrs(int pointsize, double degrees) {
    if (degrees != gl_state.currentfontrotation
...
update_transform(void) {
    ...
    trans_coord.wtos_xmult = (trans_coord.top_width - 1) / diff;
...
void drawtext_in(const t_bound_box& bbox, const std::string& text, bool check_fit, double value1) {
    drawtext(bbox.get_center(), text, bbox, check_fit, value1);
}

void drawtext_in(const t_bound_box& bbox, const std::string& text, float tolerance, bool check_fit, double value1) {
    drawtext(bbox.get_center(), text, bbox, tolerance, check_fit, value1);
}

void drawtext(
        const t_point& text_center, const std::string& text, const t_bound_box& bounds, float tolerance, bool check_fit, double value1) {
    ...
    drawtext(text_center, text, tolerance_bounds, check_fit, value1);
}

void drawtext(const t_point& text_center, const std::string& text, const t_bound_box& bounds, bool check_fit, double value1) {
    ...
    min_bounds *= 2;
    drawtext(text_center, text, min_bounds.x, min_bounds.y, check_fit, value1);
}

void drawtext(const t_point& text_center, const std::string& text, float boundx, float boundy, bool check_fit, double value1) {
    drawtext(text_center.x, text_center.y, text, boundx, boundy, check_fit, value1);
}

bool check_textbox(t_bound_box temp, double sdistance) {
    double tdistance = temp.get_width();
 ...
 */
void drawtext(float xc, float yc, const std::string& str_text, float boundx, float boundy, bool check_fit, double value1, int lower_font_bound) {
    ...
    bool pass = false;
    while (check_fit == true && getfontsize() > lower_font_bound && pass == false) {
        zero_font = gl_state.font_info.get_font_info(gl_state.currentfontsize, 0);
        XftTextExtentsUtf8(
                x11_state.display, zero_font, reinterpret_cast<const FcChar8*> (text), text_byte_length, &zero_extents);
        zero_approx_height = zero_extents.height;
        zero_approx_width = zero_extents.width;

        double hheight = fabs(0.9 * zero_approx_height * trans_ymult);
        double hwidth = fabs(0.9 * zero_approx_width * trans_xmult) / 2;
        t_bound_box b = t_bound_box(xc - hwidth, yc - hheight, xc + hwidth, yc + hheight);
        if (!check_textbox(b, value1)) {
            setfontsize(getfontsize() - 2);
        } else {
            pass = true;
        }
    }
    if (check_fit == true && pass == false) {
        return;
    }
    zero_font = gl_state.font_info.get_font_info(gl_state.currentfontsize, 0);
    XftTextExtentsUtf8(
    ...
    if (topBox == 0) {
        if (gl_state.disp_type == SCREEN) {
            ...
            XSetForeground(x11_state.display, x11_state.gc_menus,
                    x11_convert_to_xcolor(t_color(0, 0, 0, 255)));
            ...
            fprintf(gl_state.ps, "(%s) WHATS UP %.2f %.2f censhow\n", gl_state.statusMessage,
        ...
        if (gl_state.disp_type == SCREEN) {
#ifdef X11
            XClearWindow(x11_state.display, x11_state.textarea2);
            //TODO
            menutext(
                    x11_state.textarea_draw2,
                    (T_BOX2_WIDTH) / 2,
                    T_AREA_HEIGHT / 2 - 10,
                    gl_state.statusMessage
                    );
...
void
update_message(const string& msg, int topBox) {
    strncpy(gl_state.statusMessage, msg.c_str(), BUFSIZE);
    gl_state.statusMessage[BUFSIZE - 1] = '\0'; //Ensure null termination
    /*if (topBox == 1) draw_message(topBox);
    else if (topBox == 2) draw_message(topBox)
    else draw_message();*/
    draw_message(topBox);
#ifdef X11
    // Make this appear immediately.  Win32 does that automatically.
    XFlush(x11_state.display);
#endif // X11
}
...
void handle_zoom_in(float x, float y, void (*drawscreen) (void)) {
    //if(zoom_cap <= 0) return;
    //zoom_cap--;
...
void handle_zoom_out(float x, float y, void (*drawscreen) (void)) {
    //if (zoom_cap >= 13) return;
    //zoom_cap++;
    /* if (get_visible_world().area() > 0.25){
        return;
    }*/
...
void found_intersect(bool found) {
    found_inter = found;
}

void draw_intersection_highlight(double x, double y) {
    set_draw_mode(DRAW_NORMAL);
    //force_setcolor(16);
    update_brushes();
    setcolor(255, 0, 0, 255);
    int factor = 5;
    double height = std::log(get_visible_world().get_height());
    double comp = std::abs(height);
    if (comp <= 6) {
        height = 0.5;
    } else if (comp > 6 && comp <= 8.9) {
        height = 2.5;
    } else if (comp > 8.9 && comp <= 9.5) {
        height = 5;
    } else if (comp > 9.5 && comp <= 10) {
        height = 7.5;
    } else if (comp > 10) {
        height = 10;
    }
    if (found_inter) {
        XFillRectangle(x11_state.display, x11_state.toplevel, x11_state.gc_normal,
                xworld_to_scrn(x) - 0.5 * std::abs(factor * height),
                yworld_to_scrn(y) - 0.5 * std::abs(factor * height),
                std::abs(factor * height), std::abs(factor * height));
    }

}

void draw_street_seg(LatLon p1, LatLon p2, int size) {
...
void update_win(int x[2], int y[2], void (*drawscreen)(void)) {
    ...
    x[0] = min(x[0], trans_coord.top_width); /* Can't go under menu */
    x[1] = min(x[1], trans_coord.top_width);
    y[0] = min(y[0], trans_coord.top_height); /* Can't go under text area */
    y[1] = min(y[1], trans_coord.top_height);
...
close_graphics(void) {
    ...
    XftDrawDestroy(x11_state.textarea_draw2);
...
build_default_menu(void) {
    //TODO
    if (MWIDTH > 0) {
        int i = 0, x1 = 0, y1 = 30, bwid = 0, bheight = 0, space = 0;
        const int NUM_STANDARD_BUTTONS = 6, SEPARATOR_BUTTON_INDEX = 3;
        ...
        menu_attributes.do_not_propagate_mask = ButtonPressMask | ButtonReleaseMask | PointerMotionMask;
        ...
        strcpy(button_state.button[0].text, "Zoom In");
        strcpy(button_state.button[1].text, "Zoom Out");
        strcpy(button_state.button[2].text, "Zoom Fit");
        strcpy(button_state.button[3].text, "---1");
        strcpy(button_state.button[4].text, "Proceed");
        strcpy(button_state.button[5].text, "Exit");

        button_state.button[0].fcn = zoom_in;
        button_state.button[1].fcn = zoom_out;
        button_state.button[2].fcn = zoom_fit;
        button_state.button[3].fcn = NULL;
        button_state.button[4].fcn = proceed;
        button_state.button[5].fcn = quit;
        ...
        XUnmapWindow(x11_state.display, x11_state.menu);
        free(button_state.button);
        button_state.button = NULL;
        button_state.num_buttons = 0;
        x11_build_textarea();
...
void draw_surface(const Surface& surface, float x, float y, int type) {
    ...
    if (cairo_surface != NULL) {
        if (type == 0) {
            ...
            cairo_set_source_surface(x11_state.ctx, cairo_surface, x, y);
        } else if (type == 2) {
            cairo_set_source_surface(x11_state.ctx, cairo_surface,
                    xworld_to_scrn(x) - 25, yworld_to_scrn(y) - 45);
        } else if (type == 3) {
            cairo_set_source_surface(x11_state.ctx, cairo_surface,
                    xworld_to_scrn(x) - 15, yworld_to_scrn(y) - 40);
        }
...
void draw_surface(const Surface& surface, t_point upper_left, int type) {
    draw_surface(surface, upper_left.x, upper_left.y, type);
}
    ...
    while (true) {
...
#ifdef VERBOSE
        cout << "Got an event of type: " << report.type << " ON WINDOW " << x11_state.toplevel << endl;
        ...
        switch (report.type) {
                ...
                if (report.xbutton.window == x11_state.toplevel) {
                    ...
                    switch (report.xbutton.button) {
                        case Button1: /* Left mouse click; pass back to client program */
                            ...
                            //std::cout << "x: " << report.xbutton.x << " y: " << report.xbutton.y << '\n';
                            if (MWIDTH == 104 && report.xbutton.x >= get_visible_screen().right() - 145 &&
                                    report.xbutton.x <= get_visible_screen().right() - 104 &&
                                    report.xbutton.y >= 9 &&
                                    report.xbutton.y <= 35) {
                                MWIDTH = 0;
                                build_default_menu();
                            ...
                            } else if (MWIDTH == 0 && report.xbutton.x >= get_visible_screen().right() - 40 &&
                                    report.xbutton.x <= get_visible_screen().right() &&
                                    report.xbutton.y >= 9 &&
                                    report.xbutton.y <= 35) {
                                MWIDTH = 104;
                                build_default_menu();
                            ...
                            //FIX
                            if (opened_found_path && MWIDTH == 0 && report.xbutton.x >= get_visible_screen().right() - 40 &&
                                    report.xbutton.x <= get_visible_screen().right() &&
                                    report.xbutton.y >= get_visible_screen().bottom() - 25 &&
                                    report.xbutton.y <= get_visible_screen().bottom()) {
                                //Next direction
                                if (path_at < max_segments - 1 && max_segments > 0) {
                                    path_at++;
                                    update_message(directions[path_at]);
                                }
                            } else if (opened_found_path && MWIDTH == 104 && report.xbutton.x >= get_visible_screen().right() - 145 &&
                                    report.xbutton.x <= get_visible_screen().right() - 104 &&
                                    report.xbutton.y >= get_visible_screen().bottom() - 25 &&
                                    report.xbutton.y <= get_visible_screen().bottom()) {
                                if (path_at < max_segments - 1 && max_segments > 0) {
                            ...
                            if (opened_found_path && report.xbutton.x >= 0 &&
                                    report.xbutton.x <= 45 &&
                                    report.xbutton.y >= get_visible_screen().bottom() - 25 &&
                                    report.xbutton.y <= get_visible_screen().bottom()) {
                                if (path_at > 0) {
...
#endif
                switch (report.xbutton.button) {
                    case Button1: /* Scroll wheel released; stop panning */
                        keep_message = false;
                        panning_off();
                        break;
                    default:
            ...
            case MotionNotify:
                ...
                XFlush(x11_state.display);
                //draw_intersection_highlight(report.xmotion.x, report.xmotion.y);
                act_on_mousemove(xscrn_to_world(report.xmotion.x), yscrn_to_world(report.xmotion.y));
                break;
            case KeyPress:
#ifdef VERBOSE 
                printf("Got a KeyPress Event.\n");
                std::cout << report.xkey.keycode << '\n';
#endif
                try {
                    c = keypress_LUT.at(report.xkey.keycode);
                    empty = false;
                    last_char_shift = false;
                } catch (std::out_of_range e) {
                    //enter is 36, shift is 50, backspace is 22, (number 7) -> ampersand is 16, space is 65
                    empty = false;
                    if (report.xkey.keycode == 36) {
                        check = false;
                        if (!search_bar.empty()) {
                            search_bar_search(search_bar);
                            search_bar = "";
                        }
                    } else if (report.xkey.keycode == 22) {
                        if (!search_bar.empty()) {
                            search_bar.erase(search_bar.size() - 1);
                            update_message(search_bar, 1);
                            empty = false;
                        } else {
                            empty = true;
                        }
                        check = false;
                        last_char_shift = false;
                    } else if (report.xkey.keycode == 65) {
                        c = ' ';
                        check = true;
                        last_char_shift = false;
                    } else if (report.xkey.keycode == 50) {
                        check = false;
                        last_char_shift = true;

                    } else if (report.xkey.keycode == 16) {
                        check = false;
                        if (last_char_shift){
                ...
                if (check) {
                    search_bar = search_bar + c;
                    update_message(search_bar, 1);
                    update_message("For intersection search enter two street names (ie. Yonge Street & Bloor Street), for POI or street search enter its name (ie. Tim Hortons)");
                }
                if (empty) {
                    update_message("Type to begin searches", 1);
                }
                ...
                if (gl_state.get_keypress_input) {
                    ...
                    length = XLookupString(&report.xkey, keyb_buffer, max_bytes, &keysym,
                            &composestatus);
#ifdef VERBOSE 
                    std::cout << "char: " << keyb_buffer[0] << " char as int: " << (int) keyb_buffer[0]
                            << " keysym: " << keysym << endl;
#endif            
...
void x11_build_textarea2(void) {
    ...
    x11_state.textarea2 = XCreateSimpleWindow(x11_state.display, x11_state.toplevel, 0,
            T_AREA_HEIGHT / 2 - 17, T_BOX2_WIDTH, T_AREA_HEIGHT - 20, 1, x11_convert_to_xcolor(t_color::predef_colors[BLACK]),
            x11_convert_to_xcolor(t_color::predef_colors[WHITE]));

    x11_state.textarea_draw2 = XftDrawCreate(
            x11_state.display,
            x11_state.textarea2,
            x11_state.visual_info.visual,
            x11_state.colormap_to_use
    ...
    menu_attributes.do_not_propagate_mask = ButtonReleaseMask | PointerMotionMask;
    /* Keep text area on bottom left */
    menu_attributes.win_gravity = NorthWestGravity;
    valuemask = CWWinGravity | CWEventMask | CWDontPropagate;
    XChangeWindowAttributes(x11_state.display, x11_state.textarea2, valuemask, &menu_attributes);
    XMapWindow(x11_state.display, x11_state.textarea2);
}
...
/*void x11_build_textarea3(bool draw) {
    ...
    unsigned long valuemask;
    if (draw) {
        x11_state.textarea3 = XCreateSimpleWindow(x11_state.display, x11_state.toplevel, get_visible_screen().get_center().x,
                get_visible_screen().bottom() - (2 * T_AREA_HEIGHT), get_visible_screen().right() - MWIDTH - 20,
                2 * T_AREA_HEIGHT - 15, 1, x11_convert_to_xcolor(t_color::predef_colors[BLACK]),
                x11_convert_to_xcolor(t_color::predef_colors[WHITE]));

        x11_state.textarea3 = XCreateSimpleWindow(x11_state.display, x11_state.toplevel, 0,
            trans_coord.top_height - T_AREA_HEIGHT, trans_coord.display_width - MWIDTH,
            T_AREA_HEIGHT, 0, x11_convert_to_xcolor(t_color::predef_colors[RED]),
            x11_convert_to_xcolor(t_color::predef_colors[WHITE]));
        ...
        menu_attributes.win_gravity = NorthWestGravity;
        valuemask = CWWinGravity | CWEventMask | CWDontPropagate;
        XChangeWindowAttributes(x11_state.display, x11_state.textarea3, valuemask, &menu_attributes);
        if (x11_state.textarea3)
            XMapWindow(x11_state.display, x11_state.textarea3);
    }
    //TODO CHANGE COORDINATES
    if (!draw) {
        //XftDrawDestroy(x11_state.textarea_draw3);
        //XUnmapWindow(x11_state.display, x11_state.textarea3);
        //XDestroyWindow(x11_state.display, x11_state.textarea3);
    }
}*/

//TODO CHECK IF FREED VALUES
...
static void x11_drawbut(int bnum) {
    ...
    if (ispressed) {
        XSetForeground(x11_state.display, x11_state.gc_menus,
                x11_convert_to_xcolor(t_color::predef_colors[BLUE]));
    } else {
        XSetForeground(x11_state.display, x11_state.gc_menus,
                x11_convert_to_xcolor(t_color::predef_colors[BLACK]));
    }
...
static void x11_drawmenu(void) {
    ...
    if (MWIDTH > 0) {
        ...
        XSetForeground(x11_state.display, x11_state.gc_menus,
                x11_convert_to_xcolor(t_color(192, 192, 192, 255)));
